<template>
  <div>
    <Header />
    <div
      class="container mx-auto flex items-center h-[80vh] justify-between space-x-[222px]"
    >
      <div class="w-[50%] flex items-center space-x-[16px]">
        <img :src="logo" alt="Logo" />
        <h1
          class="uppercase text-black font-medium text-[28px] w-[500px] leading-[120%]"
        >
      ...
      <div class="w-[50%] max-w-[350px] mx-auto space-y-[48px]">
        ...
        <form @submit.prevent="loginUser">
          <div class="mb-4">
            <label
              class="block mb-2 text-sm font-bold text-[#265098]"
              for="email"
            >
              {{ $t("formlogin") }}
            </label>
        ...
        <div class="flex items-center justify-center">
          <router-link
            to="/email_reset"
            class="text-[12px] cursor-pointer text-[#178BF1] font-bold"
            >Parol esingizdan chiqdimi?</router-link
          >
          <span class="text-[12px] px-2">yoki</span>
          <router-link
            to="/register"
            class="text-[12px] text-[#178BF1] font-bold"
  ...
  useAuth(user)
    .then((res) => {
      ...
      if (res?.data?.data?.role === "ADMIN") {
        ...
        res?.data?.data?.role === "LIMITED_USER"
      ) {
        toast.success("Success", { autoClose: 500 });
